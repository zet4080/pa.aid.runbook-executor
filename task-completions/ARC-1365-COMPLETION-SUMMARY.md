...
- **Branch:** ARC-1365
- **Commits:** `5af49a3` (feat), `30b4410` (fix — event capture test coverage)
...
### Server-side capture (event capture fix — commit `30b4410`)
- `extractLastAgentMessage()` in `laneRunner.ts` correctly walks `result.events` for the last `TextEvent`.
  `subprocessManager.ts` (which populates `result.events` from the NDJSON stdout stream) was always correct.
- Root cause of the "bug": all prior `laneRunner.test.ts` mocks used `events: []` — no test ever
  passed a real `TextEvent` through the checkpoint creation paths, so `lastAgentMessage` was always
  `undefined` in tests and the behaviour was unverified.
- Fix: added 6 targeted tests to `laneRunner.test.ts` that pass realistic event sequences
  (`[step_start, text_event, step_finish]`) and assert `CheckpointQueueEntry.lastAgentMessage`
  is correctly populated at both checkpoint creation sites (sub-step gate and retry-exhausted).

## Test results
- **Server:** 299 tests, 19 test files — all PASSED (6 new tests added in fix commit)
- **UI:** 192 tests, 19 test files — all PASSED
- **New tests:** 7 in `AgentMessageModal.test.tsx` + 8 new cases in `ResumeCard.test.tsx` + 6 in `laneRunner.test.ts`
...
- `packages/server/src/__tests__/laneRunner.test.ts` — added 6 ARC-1365 tests with realistic event sequences
